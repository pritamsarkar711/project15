...
/**
 * ISO 3166-1 alpha-2 country list used by the author/user profile form and
 * the public profiles (name + flag icon everywhere a country is shown).
 *
 * The stored value on the user is the TWO-LETTER CODE (e.g. "BD", "US"),
 * which keeps the database clean and lets us:
 *   - render flag icons from flagcdn.com via the lowercase code
 *   - print the full country name from this list
 *
 * The list is intentionally static (no DB table): it almost never changes,
 * needs zero migrations and works on shared hosting without seeding.
 */
class Countries
{
    /** @var array<string, string> ISO alpha-2 code => English name */
    public const ALL = [
        ...
        'MO' => 'Macao', 'MQ' => 'Martinique', 'MR' => 'Mauritania',
        ...
        'PY' => 'Paraguay', 'QA' => 'Qatar', 'RE' => 'Réunion',
    ...
    /**
     * Full country name for a stored code ("BD" → "Bangladesh").
     * Returns null for empty / unknown codes so callers can hide the field.
     */
    ...
    /**
     * Flag icon URL for an ISO alpha-2 code, served by the free flagcdn.com
     * CDN (no key, no rate limit). Emoji flags are avoided on purpose —
     * Windows renders them as plain letters (e.g. "BD"), so an image icon is
     * the only cross-platform-reliable option.
     */
    public static function flagUrl(?string $code, string $size = 'w40'): ?string
    {
