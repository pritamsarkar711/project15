    ...
    <div class="card-elev p-7 sm:p-8">
        ...
        @if(\App\Models\Setting::get('google_enabled') === '1' && \App\Models\Setting::get('google_client_id'))
            <div class="relative my-6">
                <div class="absolute inset-0 flex items-center"><div class="w-full border-t border-[#e6e8ee] dark:border-[#262a33]"></div></div>
                <div class="relative flex justify-center text-xs uppercase"><span class="card-elev px-2 text-slate-400 dark:text-slate-500 uppercase tracking-wider !border-0 !shadow-none font-semibold">Or</span></div>
            </div>
            <a href="{{ route('auth.google.redirect') }}" class="btn btn-outline w-full !h-11">
        ...
        <p class="text-sm text-slate-600 dark:text-slate-400 text-center mt-6">
            New to Huvanti?
            <a href="{{ route('register') }}" class="font-semibold text-[#047a43] dark:text-emerald-300 hover:underline underline-offset-4">Create an account</a>
        </p>
        <p class="text-[11px] text-slate-500 dark:text-slate-500 text-center mt-3">
            <a href="{{ route('password.request') }}" class="hover:text-[#047a43] dark:hover:text-emerald-300">Forgot your password?</a>
        </p>
    </div>
</div>
@endsection
