...
<div class="max-w-[460px] mx-auto px-4 py-12">
    <div class="bg-white dark:bg-[#141815] border-2 border-[#141A16] dark:border-[#3A443D] shadow-[8px_8px_0_0_#F5C445] p-7 sm:p-8">
        <div class="text-center mb-7">
            ...
            <h1 class="font-black text-[26px] mt-3 text-[#141A16] dark:text-[#F0F2EB] tracking-tight">Welcome back</h1>
        ...
        <form method="POST" action="{{ route('login.post') }}" class="space-y-4">
            ...
            <label class="flex items-center gap-2 text-[13.5px] font-medium text-[#3D463F] dark:text-[#C2C9C0]">
                <input type="checkbox" name="remember" class="border-[#D8D8CC] dark:border-[#3A443D] accent-[#0C3B2E] dark:accent-[#34D399] w-4 h-4">
                Keep me signed in
            </label>

            @if(session('show_2fa'))
                <div>
                    <label class="text-[11px] font-extrabold uppercase tracking-[0.12em] text-[#141A16] dark:text-[#EDEFEA]">Two factor code</label>
                    <input type="text" name="two_factor_code" inputmode="numeric" maxlength="6" autocomplete="one-time-code" required
                        class="field mt-1.5 h-11 px-3 text-sm font-mono tracking-widest text-center placeholder:tracking-normal placeholder:font-sans"
                        placeholder="123456">
                </div>
            @endif
        ...
        <p class="text-[13.5px] font-medium text-[#5C665E] dark:text-[#97A199] text-center mt-6">
            New to Huvanti?
            <a href="{{ route('register') }}" class="font-extrabold text-[#0C3B2E] dark:text-[#34D399] hover:underline underline-offset-4">Create an account</a>
        </p>
        <p class="text-[11.5px] text-[#8B958C] dark:text-[#6B756C] text-center mt-3">
            <a href="{{ route('password.request') }}" class="hover:text-[#0C3B2E] dark:hover:text-[#34D399] hover:underline underline-offset-4">Forgot your password?</a>
        </p>
    </div>
</div>
@endsection
