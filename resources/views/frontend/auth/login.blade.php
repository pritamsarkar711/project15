...
<div class="max-w-[460px] mx-auto px-4 py-12">
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl p-7 sm:p-8">
        ...
        @endif
        @if(session('success'))
            <div class="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-800 dark:text-emerald-300 px-4 py-3 text-sm mb-4 rounded-xl">{{ session('success') }}</div>
        @endif
        ...
        <form method="POST" action="{{ route('login.post') }}" class="space-y-4">
            ...
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                <input type="checkbox" name="remember" class="rounded border-slate-300 dark:border-slate-700 text-[#05B762] focus:ring-[#05B762]">
                Keep me signed in
            </label>
            ...
            @if(session('show_2fa'))
                <div>
                    ...
                    <input type="text" name="two_factor_code" inputmode="numeric" maxlength="6" autocomplete="one-time-code" required
                        class="mt-1 w-full h-11 px-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:border-[#05B762] focus:ring-4 focus:ring-[#05B762]/15 outline-none text-sm font-mono tracking-widest text-center placeholder:tracking-normal placeholder:font-sans text-slate-900 dark:text-white"
                        placeholder="123456">
                </div>
            @endif
        ...
        @if(\App\Models\Setting::get('google_enabled') === '1' && \App\Models\Setting::get('google_client_id'))
            ...
            <a href="{{ route('auth.google.redirect') }}" class="w-full h-11 rounded-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-medium text-sm flex items-center justify-center gap-2.5 transition">
        ...
        <p class="text-sm text-slate-600 dark:text-slate-400 text-center mt-6">
            New to Huvanti?
            <a href="{{ route('register') }}" class="font-semibold text-[#049A53] dark:text-emerald-300 hover:underline">Create an account</a>
        </p>
        <p class="text-[11px] text-slate-500 dark:text-slate-500 text-center mt-3">
            <a href="{{ route('password.request') }}" class="hover:text-[#049A53] dark:hover:text-emerald-300">Forgot your password?</a>
        </p>
    </div>
</div>
@endsection
