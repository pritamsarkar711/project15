...
<div class="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
    <div class="flex items-center gap-4 mb-8 card-elev p-5">
        <span class="icon-tile w-14 h-14">
            ...
            @foreach($posts as $p)
                <article class="group card-elev card-hover overflow-hidden flex flex-col">
                    ...
                    <div class="p-5 flex flex-col flex-1">
                        <a href="{{ route('blog.show',$p->slug) }}" class="font-semibold text-slate-900 dark:text-white hover:text-[#049A53] dark:hover:text-emerald-300 transition-colors line-clamp-2">{{ $p->title }}</a>
                        <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mt-2">{{ $p->excerpt }}</p>
                        <div class="text-xs text-slate-400 dark:text-slate-500 mt-3 pt-3 border-t border-slate-100 dark:border-[#2f2f2f]">{{ $p->published_at->format('M d, Y') }} <span class="w-1 h-1 bg-slate-300 dark:bg-slate-600 inline-block mx-1 align-middle rounded-full"></span> {{ $p->reading_time }} min read</div>
                    </div>
                </article>
            @endforeach
        </div>
