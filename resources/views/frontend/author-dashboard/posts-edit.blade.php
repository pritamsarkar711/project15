    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                ...
                @elseif($post->review_status === 'approved') <span class="font-semibold text-[#27654A] dark:text-[#57A37E]">Published</span>
                @endif
            </p>
        </div>
        @if($post->review_status === 'approved')
            <a href="{{ route('blog.show', $post->slug) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 h-9 px-4 rounded-lg bg-[#2E7856] hover:bg-[#27654A] text-white text-xs font-semibold">
