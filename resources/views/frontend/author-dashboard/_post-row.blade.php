...
$reviewBadge = match($post->review_status) {
    ...
    'approved'       => '<span class="inline-flex items-center px-2 py-0.5 text-[11px] font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300">Published</span>',
    default          => '',
};
@endphp
<div class="flex items-center gap-4 p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
    ...
    <div class="flex-1 min-w-0">
        ...
        <a href="{{ route('author.posts.edit', $post->id) }}" class="block font-semibold text-slate-900 dark:text-white hover:text-[#0C3B2E] dark:hover:text-emerald-300 text-sm mt-1 truncate">{{ $post->title }}</a>
