...
$reviewBadge = match($post->review_status) {
    ...
    'approved'       => '<span class="inline-flex items-center px-2 py-0.5 text-[11px] font-semibold bg-[var(--brand-tint)] text-[var(--brand-ink)] dark:bg-[var(--brand)]/20 dark:text-[var(--brand-light)]">Published</span>',
    default          => '',
};
@endphp
<div class="flex items-center gap-4 p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
    ...
    <div class="flex-1 min-w-0">
        ...
        <a href="{{ route('author.posts.edit', $post->id) }}" class="block font-semibold text-slate-900 dark:text-white hover:text-[var(--brand-deep)] dark:hover:text-[var(--brand-light)] text-sm mt-1 truncate">{{ $post->title }}</a>
    ...
    <div class="flex flex-wrap items-center gap-1 shrink-0">
        @if($post->review_status === 'draft' || $post->review_status === 'returned')
            ...
            <form method="POST" action="{{ route('author.posts.submit', $post->id) }}">@csrf
                <button type="submit" class="px-3 h-9 inline-flex items-center text-xs font-semibold rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white">Submit</button>
            </form>
        ...
        @elseif($post->review_status === 'approved')
            ...
            <a href="{{ route('author.posts.share', $post->id) }}" class="px-3 h-9 inline-flex items-center text-xs font-semibold text-[var(--brand-deep)] dark:text-[var(--brand-light)] hover:underline gap-1">
            ...
            <a href="{{ route('blog.show', $post->slug) }}" target="_blank" class="px-3 h-9 inline-flex items-center text-xs font-semibold text-[var(--brand-deep)] dark:text-[var(--brand-light)] hover:underline">View live</a>
        @endif
    </div>
</div>
