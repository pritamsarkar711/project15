...
<div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
    ...
    <div class="panel-card p-4">
        <div class="flex items-center justify-between">
            ...
            <svg class="w-5 h-5 text-[#27654A] dark:text-[#6FB393] shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3"/></svg>
        </div>
        <div class="text-3xl font-extrabold mt-2 {{ $stats['returned'] > 0 ? 'text-amber-600 dark:text-amber-300' : '' }}">{{ $stats['returned'] }}</div>
        <a href="{{ route('author.posts.index', ['tab' => 'returned']) }}" class="mt-1 inline-flex text-xs font-semibold text-[#1F513A] dark:text-[#6FB393] hover:underline">Fix and resubmit</a>
    </div>
    <div class="panel-card p-4">
        <div class="flex items-center justify-between">
            <span class="text-[10px] font-bold tracking-widest uppercase text-slate-500 dark:text-slate-400">Published</span>
            <svg class="w-5 h-5 text-[#27654A] dark:text-[#6FB393] shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
        </div>
        <div class="text-3xl font-extrabold mt-2 text-[#27654A] dark:text-[#6FB393]">{{ $stats['published'] }}</div>
    ...
    <div class="panel-card p-4">
        <div class="flex items-center justify-between">
            ...
            <svg class="w-5 h-5 text-[#27654A] dark:text-[#6FB393] shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/></svg>
        </div>
    ...
    <div class="lg:col-span-2 space-y-5">
        ...
        <div class="panel-card p-5">
            <div class="flex items-start gap-3">
                <div class="w-10 h-10 bg-[#F0F7F3] dark:bg-[#2E7856]/10 text-[#1F513A] dark:text-[#6FB393] flex items-center justify-center shrink-0">
        ...
        <div class="panel-card p-5">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('author.posts.index') }}" class="text-xs font-semibold text-[#1F513A] dark:text-[#6FB393] hover:underline">View all</a>
            </div>
            @if($recent->isEmpty())
                <div class="text-center py-8">
                    <div class="w-12 h-12 mx-auto bg-[#F0F7F3] dark:bg-[#2E7856]/10 text-[#1F513A] dark:text-[#6FB393] flex items-center justify-center">
                    ...
                    <a href="{{ route('author.posts.create') }}" class="inline-flex items-center gap-2 mt-4 h-10 px-5 rounded-lg bg-[#2E7856] hover:bg-[#27654A] text-white text-sm font-semibold">
    ...
    <div>
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-[0_1px_4px_rgba(15,23,42,0.05)]">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2.5">
                <a href="{{ route('author.posts.create') }}" class="group flex flex-col items-start gap-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 p-3.5 hover:border-[#2E7856]/50 hover:bg-white dark:hover:bg-slate-800 transition">
                ...
                <a href="{{ route('author.posts.index') }}" class="group flex flex-col items-start gap-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 p-3.5 hover:border-[#2E7856]/50 hover:bg-white dark:hover:bg-slate-800 transition">
                    <span class="icon-tile w-8 h-8 rounded-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
                    </span>
                    <span class="text-sm font-semibold text-slate-700 dark:text-slate-200 group-hover:text-[#27654A] dark:group-hover:text-[#6FB393] transition">My Posts</span>
                </a>
                <a href="{{ route('author.rules') }}" class="group flex flex-col items-start gap-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 p-3.5 hover:border-[#2E7856]/50 hover:bg-white dark:hover:bg-slate-800 transition">
                    <span class="icon-tile w-8 h-8 rounded-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25"/></svg>
                    </span>
                    <span class="text-sm font-semibold text-slate-700 dark:text-slate-200 group-hover:text-[#27654A] dark:group-hover:text-[#6FB393] transition">Posting Rules</span>
                </a>
                <a href="{{ route('author.profile.edit') }}" class="group flex flex-col items-start gap-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 p-3.5 hover:border-[#2E7856]/50 hover:bg-white dark:hover:bg-slate-800 transition">
                    <span class="icon-tile w-8 h-8 rounded-lg">
        ...
        <div class="panel-card p-5 mt-5">
            ...
            <ul class="mt-3 space-y-2 text-sm text-slate-600 dark:text-slate-400">
                <li class="flex gap-2"><span class="text-[#27654A] dark:text-[#6FB393] font-bold">1</span> Write at least 300 words.</li>
                <li class="flex gap-2"><span class="text-[#27654A] dark:text-[#6FB393] font-bold">2</span> Add a featured image.</li>
                <li class="flex gap-2"><span class="text-[#27654A] dark:text-[#6FB393] font-bold">3</span> Answer a real reader question.</li>
            </ul>
        </div>
    </div>
</div>
