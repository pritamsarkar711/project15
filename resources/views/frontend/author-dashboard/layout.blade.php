...
<body class="panel-ui min-h-screen bg-slate-100 text-slate-900 dark:bg-[#0f172a] dark:text-slate-100 flex overflow-x-hidden" style="font-family:{{ \App\Support\SiteFont::cssStack() }}">
    ...
    <aside id="author-sidebar" class="fixed inset-y-0 left-0 w-[250px] bg-slate-900 dark:bg-slate-950 text-slate-300 flex flex-col z-40 transform lg:translate-x-0 -translate-x-full transition-transform duration-300 overflow-y-auto no-scrollbar border-r border-black/30">
        ...
        <nav class="flex-1 p-3 space-y-1 text-sm font-medium">
            <a href="{{ route('author.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg transition {{ request()->routeIs('author.dashboard') ? 'bg-[#0C3B2E] text-white' : 'hover:bg-white/5 hover:text-white' }}">
            ...
            <a href="{{ route('author.posts.index') }}" class="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg transition {{ request()->routeIs('author.posts.index') ? 'bg-[#0C3B2E] text-white' : 'hover:bg-white/5 hover:text-white' }}">
            ...
            <a href="{{ route('author.posts.create') }}" class="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg transition {{ request()->routeIs('author.posts.create') ? 'bg-[#0C3B2E] text-white' : 'hover:bg-white/5 hover:text-white' }}">
            ...
            <a href="{{ route('author.rules') }}" class="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg transition {{ request()->routeIs('author.rules') ? 'bg-[#0C3B2E] text-white' : 'hover:bg-white/5 hover:text-white' }}">
                <svg class="w-[18px] h-[18px] shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25"/></svg>
                Posting Rules
            </a>
            <a href="{{ route('author.feedback.index') }}" class="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg transition {{ request()->routeIs('author.feedback*') ? 'bg-[#0C3B2E] text-white' : 'hover:bg-white/5 hover:text-white' }}">
            ...
            <a href="{{ route('author.profile.edit') }}" class="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg transition {{ request()->routeIs('author.profile*') ? 'bg-[#0C3B2E] text-white' : 'hover:bg-white/5 hover:text-white' }}">
            ...
            <a href="{{ route('author.revenue') }}" class="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg transition {{ request()->routeIs('author.revenue') ? 'bg-[#0C3B2E] text-white' : 'hover:bg-white/5 hover:text-white' }}">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                @if(auth()->check() && auth()->user()->browsingAsUser())
                    <form method="POST" action="{{ route('switch-back-to-admin') }}" class="inline">
                        @csrf
                        <button type="submit" title="Return to the admin panel" aria-label="Switch to Admin" class="inline-flex items-center justify-center gap-2 h-9 px-3 sm:px-4 text-sm font-semibold bg-[#05B762] hover:bg-[#049A53] rounded-full text-white transition cursor-pointer">
                ...
                <a href="{{ url('/') }}" class="inline-flex items-center justify-center gap-2 h-9 px-3 sm:px-4 text-sm font-semibold text-white bg-[#05B762] hover:bg-[#049A53] rounded-full transition" aria-label="View site" title="View site">
    ...
    <button id="author-back-top" type="button" aria-label="Back to top" title="Back to top"
        class="fixed bottom-4 right-4 w-10 h-10 bg-[#05B762] hover:bg-[#049A53] rounded-full dark:bg-emerald-400 dark:hover:bg-emerald-500 dark:text-slate-900 text-white shadow-lg flex items-center justify-center transition-opacity duration-200 opacity-0 pointer-events-none z-30">
