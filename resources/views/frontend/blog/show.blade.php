    ...
    <div class="flex items-center justify-center gap-3 mt-5">
        @auth
            <form method="POST" action="{{ route('blog.react', $post->slug) }}" class="inline">
                ...
                <button type="submit" title="{{ $myReaction === 'like' ? 'Remove your like' : 'Like this post' }}" class="inline-flex items-center gap-2 h-11 px-5 rounded-lg text-sm font-bold transition cursor-pointer border {{ $myReaction === 'like' ? 'bg-[#05B762] border-[#05B762] text-white shadow-sm' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-[#05B762] hover:text-[#049A53] dark:hover:text-emerald-300' }}">
                        ...
                        <div class="flex items-center gap-2 mt-3 flex-wrap">
                            ...
                            @if(auth()->check() && $post->user && auth()->id() !== $post->user->id)
                                <form method="POST" action="{{ route('author.follow', $post->user->username) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center gap-1.5 h-9 px-4 text-xs font-semibold rounded-lg border transition cursor-pointer {{ $followingAuthor ? 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700' : 'bg-white dark:bg-[#2a2a2a] text-[#049A53] dark:text-emerald-300 border-[#05B762] dark:border-emerald-400/40 hover:bg-[#05B762]/5 dark:hover:bg-emerald-400/10' }}">
