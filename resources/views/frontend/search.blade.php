...
<div class="border-b border-[#e6e8ee] dark:border-[#22262e] bg-white dark:bg-[#0f1115]">
    <div class="max-w-[1200px] mx-auto px-4 sm:px-6 page-head !pb-6">
        ...
        <h1>Search results @if($q)for <span class="text-[#047a43] dark:text-emerald-300">“{{ $q }}”</span>@endif</h1>
        ...
        @forelse($posts as $p)
            <article class="group card-elev card-hover overflow-hidden flex flex-col">
                ...
                <div class="p-5 flex flex-col flex-1">
                    <a href="{{ route('blog.show',$p->slug) }}" class="text-[16px] font-bold text-slate-900 dark:text-white leading-snug tracking-[-0.01em] line-clamp-2 group-hover:text-[#047a43] dark:group-hover:text-emerald-300 transition-colors">{{ $p->title }}</a>
