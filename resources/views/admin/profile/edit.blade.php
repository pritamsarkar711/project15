...
<div class="w-full space-y-6">
    ...
    <form method="POST" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" class="panel-card p-6 space-y-5">
        ...
        <div class="flex items-center gap-4">
            ...
            @else
                ...
                <div id="avatar-initial" class="w-20 h-20 rounded-full bg-[var(--brand-deep)] text-white flex items-center justify-center text-2xl font-extrabold">{{ strtoupper(substr($user->name,0,1)) }}</div>
            @endif
            <div>
        ...
        <button type="submit" class="h-11 px-6 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Save profile</button>
    </form>
    ...
    <form method="POST" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" class="panel-card p-6 space-y-5">
        ...
        <button type="submit" class="h-11 px-6 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Update credentials</button>
    </form>
</div>
@push('scripts')
<script>
