...
<div class="flex justify-between items-center mb-5">
    ...
    <a href="{{ route('admin.pages.create') }}" class="h-9 px-4 bg-[#0C3B2E] hover:bg-[#072A20] text-white text-sm font-semibold inline-flex items-center gap-1.5">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14m-7-7h14"/></svg> New Page
    </a>
</div>
<div class="border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse($pages as $page)
                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                        ...
                        <td class="px-4 py-3"><span class="text-xs font-semibold px-2.5 py-1 {{ $page->status=='published' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300' }}">{{ $page->status }}</span></td>
                        <td class="px-4 py-3 text-xs text-slate-500 dark:text-slate-400">{{ $page->updated_at->format('M d, Y') }}</td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-1">
                                <a href="{{ route('page.show', $page->slug) }}" target="_blank" title="View" class="w-8 h-8 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-center">
                                ...
                                <a href="{{ route('admin.pages.edit', $page) }}" title="Edit" class="w-8 h-8 bg-[#0C3B2E] hover:bg-[#072A20] text-white flex items-center justify-center">
    ...
    <div class="p-4 border-t border-slate-100 dark:border-slate-800">{{ $pages->links() }}</div>
</div>
@endsection
