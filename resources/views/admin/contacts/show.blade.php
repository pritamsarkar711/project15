...
<div class="max-w-3xl">
    ...
    <div class="border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
        <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            <span class="text-xs font-semibold px-3 py-1 {{ $contact->is_read ? 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' }}">{{ $contact->is_read ? 'Read' : 'Unread' }}</span>
        </div>
        ...
        <div class="mt-6 flex flex-wrap gap-2">
            <a href="mailto:{{ $contact->email }}?subject=Re: {{ $contact->subject ?? $contact->reason }}" class="h-10 px-5 bg-[#0C3B2E] hover:bg-[#072A20] text-white text-sm font-semibold inline-flex items-center gap-2 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 0 0 2.22 0L21 8M5 19h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2Z"/></svg>
                Reply via Email
            </a>
