...
@if(request('tab') === 'integrations')
    <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-5 w-full">
        ...
        <div class="panel-card p-6 space-y-4">
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500">Served live at <a href="{{ url('ads.txt') }}" target="_blank" class="text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">{{ url('ads.txt') }}</a></p>
        </div>
...
Disallow: /manage" class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm font-mono placeholder:font-sans">{{ old('robots_txt_content', $settings['robots_txt_content']->value ?? '') }}</textarea>
            <p class="text-xs text-slate-400 dark:text-slate-500">Served at <a href="{{ url('robots.txt') }}" target="_blank" class="text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">{{ url('robots.txt') }}</a></p>
        </div>
        ...
        <div class="panel-card p-6 space-y-4">
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500">Served at <a href="{{ url('llms.txt') }}" target="_blank" class="text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">{{ url('llms.txt') }}</a></p>
        </div>
        ...
        <div class="panel-card p-6 space-y-4">
            ...
            <p class="text-sm text-slate-600 dark:text-slate-400">
                @if($indexNowKey)
                    <span class="inline-flex items-center gap-1.5 font-semibold text-[var(--brand-ink)] dark:text-[var(--brand-light)]"><svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Active</span>
            ...
            @if($indexNowKey)
                <p class="text-xs text-slate-500">Key file: <a href="{{ url($indexNowKey.'.txt') }}" target="_blank" class="font-mono text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline break-all">{{ url($indexNowKey.'.txt') }}</a></p>
            @endif
        </div>
        ...
        <div class="panel-card p-6 space-y-4">
            <div class="flex items-center justify-between">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-sm font-medium {{ ($settings['google_enabled']->value ?? '0') === '1' ? 'text-[var(--brand-ink)] dark:text-[var(--brand-light)]' : 'text-slate-500' }}">{{ ($settings['google_enabled']->value ?? '0') === '1' ? 'Enabled' : 'Disabled' }}</span>
                </label>
            </div>
        ...
        <button type="submit" class="h-11 px-6 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Save Integrations</button>
    </form>
...
@elseif(request('tab') === 'appearance')
    <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-5 w-full">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
                ...
                @foreach(\App\Support\SiteThemes::all() as $key => $theme)
                    <label class="relative flex cursor-pointer">
                        <input type="radio" name="site_theme" value="{{ $key }}" class="peer sr-only" @checked($currentTheme === $key)>
                        <span class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/40 p-4 transition peer-checked:border-[var(--brand)] peer-checked:ring-2 peer-checked:ring-[var(--brand)]/25 hover:border-slate-300 dark:hover:border-slate-600">
                            <span class="flex items-center gap-2.5">
                                <span class="flex -space-x-1.5">
                                    @foreach($theme['swatch'] as $i => $hex)
                                        <span class="w-6 h-6 rounded-full border-2 border-white dark:border-slate-800 inline-block" style="background:{{ $hex }}; z-index:{{ 3 - $i }}"></span>
                                    @endforeach
                                </span>
                                <span class="font-semibold text-sm text-slate-900 dark:text-white">{{ $theme['label'] }}</span>
                                @if($currentTheme === $key)
                            ...
                            <span class="block text-xs text-slate-500 dark:text-slate-400 mt-2">{{ $theme['hint'] }}</span>
                        </span>
                    </label>
                @endforeach
            </div>
        ...
        <button type="submit" class="h-11 px-6 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Save</button>
    </form>
        ...
        <div class="panel-card p-6 space-y-4">
            ...
            <div class="grid sm:grid-cols-2 gap-5 items-start">
                <div>
                    ...
                    <label for="hero_person_image_file" class="mt-2 flex items-center justify-center h-24 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg cursor-pointer hover:border-[var(--brand-deep)] hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                    ...
                    <p class="mt-2 text-xs font-medium text-[var(--brand-ink)] dark:text-[var(--brand-light)] hidden" id="hero-file-name"></p>
                ...
                <div>
                    ...
                    <div class="mt-2 w-32 h-32 rounded-xl overflow-hidden bg-[var(--brand-deep)] flex items-center justify-center relative">
...
@elseif(request('tab') === 'ads')
    <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-5 w-full">
        ...
        <div class="panel-card p-6 space-y-4">
            <div class="flex items-center justify-between gap-4 flex-wrap">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-sm font-semibold {{ ($settings['ads_enabled']->value ?? '0') === '1' ? 'text-[var(--brand-ink)] dark:text-[var(--brand-light)]' : 'text-slate-500 dark:text-slate-400' }}">{{ ($settings['ads_enabled']->value ?? '0') === '1' ? 'Enabled' : 'Disabled' }}</span>
                </label>
            </div>
        </div>
        ...
        <div class="panel-card p-6 space-y-4">
            ...
            <p class="text-xs text-slate-500 mt-2">Manage ads on the <a href="{{ route('admin.ads.index') }}" class="text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">Advertisements page</a>.</p>
        </div>
        ...
        <div class="panel-card p-6 space-y-4">
            <div class="flex items-center justify-between gap-4 flex-wrap">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-sm font-semibold {{ ($settings['revenue_enabled']->value ?? '0') === '1' ? 'text-[var(--brand-ink)] dark:text-[var(--brand-light)]' : 'text-slate-500 dark:text-slate-400' }}">{{ ($settings['revenue_enabled']->value ?? '0') === '1' ? 'Enabled' : 'Disabled' }}</span>
                </label>
            </div>
        </div>
        <button type="submit" class="h-11 px-6 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Save Ad Settings</button>
    </form>
...
@elseif(request('tab') === 'email')
    ...
    <form method="POST" action="{{ route('admin.settings.smtp.update') }}" class="space-y-5 w-full">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <label class="mt-2 inline-flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300 cursor-pointer">
                        <input type="checkbox" name="mail_remove_password" value="1" class="text-[var(--brand-strong)]">
                        Remove stored password
                    </label>
                </div>
            </div>
            <p class="text-xs text-slate-500 mt-1 pt-2 border-t border-slate-200 dark:border-slate-700">
                Gmail: use an <a href="https://myaccount.google.com/apppasswords" target="_blank" rel="noopener" class="text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">App Password</a>.
            </p>
        </div>
        ...
        <button type="submit" class="h-11 px-6 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Save SMTP Settings</button>
    </form>
        ...
        <div class="panel-card p-6 space-y-4">
            <div class="flex items-center justify-between">
                ...
                <label class="inline-flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300 cursor-pointer">
                    <input type="checkbox" name="social_enabled" value="1" {{ old('social_enabled', $settings['social_enabled']->value ?? '1') === '1' ? 'checked' : '' }} class="text-[var(--brand-strong)]">
                    Show in footer
                </label>
            </div>
        ...
        <div class="panel-card p-6">
            <div class="flex items-center justify-between gap-4">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer shrink-0">
                    ...
                    <span class="text-sm font-medium {{ $tcOn ? 'text-[var(--brand-ink)] dark:text-[var(--brand-light)]' : 'text-slate-500' }}">{{ $tcOn ? 'Enabled' : 'Disabled' }}</span>
                </label>
            </div>
        </div>
        ...
        <div class="border {{ $maintenanceOn ? 'border-amber-400 dark:border-amber-500/60' : 'border-slate-200 dark:border-slate-800' }} bg-white dark:bg-slate-900 p-6 space-y-3">
            <div class="flex items-center justify-between gap-4">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer shrink-0">
                    ...
                    <span class="text-sm font-medium {{ $maintenanceOn ? 'text-[var(--brand-ink)] dark:text-[var(--brand-light)]' : 'text-slate-500' }}">{{ $maintenanceOn ? 'Enabled' : 'Disabled' }}</span>
                </label>
            </div>
            <div>
        ...
        <div class="panel-card p-6 space-y-4">
            ...
            <div class="grid sm:grid-cols-3 gap-5">
                <div>
                    ...
                    <label class="mt-2 flex items-center justify-center h-16 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg cursor-pointer hover:border-[var(--brand-deep)] hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
        ...
        heroInput.addEventListener('change', function () {
            if (heroInput.files && heroInput.files[0]) {
                ...
                if (!file.type.startsWith('image/') || file.type === 'image/heic' || file.type === 'image/heif' || file.type === 'image/avif') {
                    ...
                    heroName.classList.remove('hidden', 'text-[var(--brand-ink)]', 'dark:text-[var(--brand-light)]');
                ...
                if (heroName) {
                    ...
                    heroName.classList.add('text-[var(--brand-ink)]', 'dark:text-[var(--brand-light)]');
                }
            }
        });
    }
    ...
    document.querySelectorAll('input[type="file"][data-preview]').forEach(function (input) {
        input.addEventListener('change', function () {
            ...
            if (file.type === 'image/svg+xml' || file.type === 'image/x-icon') {
                ...
                else if (wrap) { wrap.innerHTML = '<span class="text-xs font-medium text-[var(--brand-ink)] dark:text-[var(--brand-light)]">' + file.name + ' selected. Click Save.</span>'; }
                return;
            }
