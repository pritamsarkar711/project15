...
<div class="w-full">
    ...
    <form method="POST" action="{{ route('admin.categories.store') }}" class="panel-card p-6 space-y-5">
        ...
        <div class="grid sm:grid-cols-2 gap-4 items-end">
            ...
            <label class="flex items-center gap-2 text-sm h-10"><input type="checkbox" name="is_active" value="1" checked class="border-slate-300 dark:border-slate-600 text-[#27654A]"> Active (visible on site)</label>
        </div>
        <div class="flex gap-2">
            <button type="submit" class="h-11 px-6 rounded-lg bg-[#2E7856] hover:bg-[#27654A] text-white font-semibold transition">Create Category</button>
