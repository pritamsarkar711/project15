...
<div class="w-full">
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="panel-card p-6 space-y-5">
        ...
        <div class="grid sm:grid-cols-2 gap-4 items-end">
            ...
            <label class="flex items-center gap-2 text-sm h-10"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $category->is_active)) class="border-slate-300 dark:border-slate-600 text-[var(--brand-strong)]"> Active (visible on site)</label>
        </div>
        <div class="flex gap-2">
            <button type="submit" class="h-11 px-6 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Save Changes</button>
