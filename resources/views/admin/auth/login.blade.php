...
<body class="min-h-screen bg-slate-100 dark:bg-[#0f172a] flex items-center justify-center p-4" style="font-family:{{ \App\Support\SiteFont::cssStack() }}">
    <div class="w-full max-w-[400px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm p-7 sm:p-8 text-slate-900 dark:text-slate-100">
        <div class="text-center mb-7">
            <div class="w-12 h-12 bg-[var(--brand-deep)] rounded-xl flex items-center justify-center mx-auto text-white" aria-hidden="true">
        ...
        @endif
        @if(session('success'))
            <div class="bg-[var(--brand-tint-3)] dark:bg-[var(--brand)]/10 border border-[var(--brand-tint-2)] dark:border-[var(--brand)]/30 text-[var(--brand-deep)] dark:text-[var(--brand-light)] px-4 py-3 text-sm mb-4">{{ session('success') }}</div>
        @endif
        ...
        <form method="POST" action="{{ route('admin.login.post') }}" class="space-y-4">
            ...
            <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                <input type="checkbox" name="remember" class="border-slate-300 dark:border-slate-600 text-[var(--brand-strong)]"> Remember me
            </label>
            <button type="submit" class="w-full h-12 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Sign In</button>
        </form>
