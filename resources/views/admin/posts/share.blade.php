...
<div class="max-w-3xl mx-auto space-y-5">
    <div class="panel-card p-6 sm:p-8 text-center">
        <div class="w-14 h-14 rounded-full bg-[var(--brand-tint)] dark:bg-[var(--brand-tint-dark)] text-[var(--brand-ink)] dark:text-[var(--brand-light)] flex items-center justify-center mx-auto mb-4">
        ...
        <div class="mt-5 flex items-stretch gap-2 max-w-xl mx-auto">
            ...
            <button type="button" id="share-copy-btn"
                    class="h-11 px-4 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold text-sm transition whitespace-nowrap">
                Copy link
            </button>
    ...
    <div class="panel-card p-6">
            ...
            <div class="flex items-center gap-3">
                @if($autopostEnabled)
                    <form method="POST" action="{{ route('admin.social.push', $post) }}">@csrf
                        <button type="submit" class="h-8 px-3 text-xs font-semibold rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white transition">Push to socials now</button>
                    </form>
                @endif
                <a href="{{ route('admin.social.index') }}" class="text-xs font-semibold text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">Manage automation →</a>
            </div>
        </div>
        @if(!$autopostEnabled)
            <p class="text-sm text-slate-500 dark:text-slate-400">Auto-posting is <strong>off</strong>. Enable it in <a href="{{ route('admin.social.index') }}" class="text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline font-medium">Social Auto-Post</a> to share new posts automatically.</p>
        @elseif($activeNetworks && count($activeNetworks))
                        ...
                        <li class="flex items-center justify-between gap-3 text-sm border border-slate-200 dark:border-slate-800 p-3">
                            ...
                            <span class="flex items-center gap-2 min-w-0">
                                @if($row->status === 'success')
                                    <span class="inline-flex items-center gap-1 text-[var(--brand-ink)] dark:text-[var(--brand-light)] font-semibold text-xs"><svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/></svg>Published</span>
                                    @if($row->external_url)<a href="{{ $row->external_url }}" target="_blank" rel="noopener" class="text-xs text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">view</a>@endif
            ...
            @endif
        @else
            <p class="text-sm text-slate-500 dark:text-slate-400">No network is configured yet. Add credentials in <a href="{{ route('admin.social.index') }}" class="text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline font-medium">Social Auto-Post</a>.</p>
        @endif
    </div>
    ...
    <div class="flex gap-3 justify-center pb-2">
        ...
        <a href="{{ route('admin.posts.create') }}" class="h-10 px-5 inline-flex items-center rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold text-sm transition">Write another post</a>
    </div>
</div>
    ...
    (function(){
        ...
        btn.addEventListener('click', function(){
            ...
            var done = function(ok){
                ...
                btn.style.background = ok ? 'var(--brand-ink)' : '';
