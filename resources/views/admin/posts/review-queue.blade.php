            ...
            <div class="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex flex-col gap-2 shrink-0">
                        @if($post->review_status === 'pending_review' || $post->review_status === 'returned')
                            ...
                            <button type="button" data-open-review="{{ $post->id }}" class="px-3 h-9 inline-flex items-center text-xs font-semibold rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white">Review & approve</button>
                ...
                <div data-review-panel="{{ $post->id }}" class="hidden mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <form method="POST" action="{{ route('admin.posts.approve', $post) }}" class="space-y-4">
                        ...
                        <button type="submit" class="h-10 px-4 bg-[var(--brand-strong)] hover:bg-[var(--brand-ink)] text-white text-xs font-semibold">Approve & publish</button>
                    </form>
