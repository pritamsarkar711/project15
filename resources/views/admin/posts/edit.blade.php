    ...
    <div class="grid lg:grid-cols-12 gap-6">
            ...
            <div class="panel-card p-6">
                ...
                <div id="faqs" class="space-y-3">
                    @forelse(old('faqs', $post->faqs->map(fn($f)=>['question'=>$f->question,'answer'=>$f->answer])->toArray()) as $idx => $faq)
                        <div class="faq-item rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/50 p-3">
                            <input type="text" name="faqs[{{ $idx }}][question]" value="{{ $faq['question'] ?? '' }}" placeholder="Question" class="h-10 w-full px-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md text-sm">
                            <textarea name="faqs[{{ $idx }}][answer]" placeholder="Answer" rows="2" class="mt-2 px-3 py-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md text-sm">{{ $faq['answer'] ?? '' }}</textarea>
                            <button type="button" onclick="removeFaq(this)" class="mt-2 text-xs font-semibold text-red-600 dark:text-red-400 hover:underline">Remove</button>
                        </div>
                    @empty
                        <div class="faq-item rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/50 p-3">
                            <input type="text" name="faqs[0][question]" placeholder="Question" class="h-10 w-full px-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md text-sm">
                            <textarea name="faqs[0][answer]" placeholder="Answer" rows="2" class="mt-2 px-3 py-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md text-sm"></textarea>
                        </div>
                    @endforelse
                </div>
                <button type="button" onclick="addFaq()" class="mt-3 text-sm font-semibold text-[var(--brand-ink)] dark:text-[var(--brand-light)] hover:underline">Add another FAQ</button>
            </div>
        ...
        <div class="lg:col-span-4 space-y-5">
            <div class="panel-card p-6">
                ...
                <label class="flex items-center gap-2 mt-3 text-sm"><input type="checkbox" name="is_featured" value="1" @checked($post->is_featured) class="border-slate-300 dark:border-slate-600 text-[var(--brand-strong)]"> Featured post</label>
                <label class="flex items-center gap-2 mt-2 text-sm"><input type="checkbox" name="allow_comments" value="1" @checked($post->allow_comments) class="border-slate-300 dark:border-slate-600 text-[var(--brand-strong)]"> Allow comments</label>
                <button type="submit" class="w-full mt-4 h-11 rounded-lg bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white font-semibold transition">Update Post</button>
            ...
            <div class="panel-card p-6">
                ...
                @endif
                <label class="block w-full cursor-pointer border border-dashed border-slate-300 dark:border-slate-600 py-3 text-center text-sm text-slate-500 dark:text-slate-400 hover:border-[var(--brand)] hover:text-[var(--brand-ink)] dark:hover:text-[var(--brand-light)] transition">
...
<script>
    ...
    function addFaq(){
        ...
        d.className='faq-item rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/50 p-3';
        d.innerHTML=`<input type="text" name="faqs[${faqIdx}][question]" placeholder="Question" class="h-10 w-full px-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md text-sm"><textarea name="faqs[${faqIdx}][answer]" placeholder="Answer" rows="2" class="mt-2 px-3 py-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md text-sm"></textarea><button type="button" class="mt-2 text-xs font-semibold text-red-600 dark:text-red-400 hover:underline">Remove</button>`;
        d.querySelector('button').onclick = function(){ this.closest('.faq-item').remove(); };
        c.appendChild(d);
        faqIdx++;
    }
