...
<head>
    ...
    <title>@yield('title','Dashboard') - Huvanti Admin</title>
    ...
    <style>*{font-family:{{ \App\Support\SiteFont::cssStack() }}}</style>
    @stack('head')
</head>
<body class="min-h-screen bg-slate-100 text-slate-900 dark:bg-[#0f172a] dark:text-slate-100 flex overflow-x-hidden">
    <!-- Sidebar -->
    <aside id="admin-sidebar" class="fixed inset-y-0 left-0 w-[250px] bg-slate-900 dark:bg-slate-950 text-slate-300 flex flex-col z-40 transform lg:translate-x-0 -translate-x-full transition-transform duration-300 overflow-y-auto no-scrollbar border-r border-black/30">
        <div class="h-[64px] flex items-center gap-3 px-5 border-b border-white/10 shrink-0">
            ...
            <div>
                ...
                <div class="text-[10px] font-semibold tracking-[0.2em] text-slate-500">ADMIN</div>
            </div>
        </div>
    ...
    <div class="flex-1 lg:ml-[250px] min-w-0 flex flex-col min-h-screen">
        ...
        <footer class="px-6 py-3 text-center text-[11px] font-medium tracking-wide text-slate-400 dark:text-slate-600 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">Huvanti CMS</footer>
    </div>
